...
def convert_ics_event(ics_event, community_id):
    ...
    # Generate unique ID using full hash and ics prefix
    unique_id = f"{ics_event['summary']}{ics_event['start']}{ics_event.get('uid', '')}".encode()
    event_id = f"evt_ics_{community_id}_{hashlib.md5(unique_id).hexdigest()}"
