...
BASE_URL = "https://history.columbia.edu/events"
...
def scrape_columbia_history_events() -> Dict[str, List[Dict]]:
	...
	for h in headers:
		...
		events.append({
			"id": f"evt_columbia_history_{uid}",
			"name": title,
			"description": description,
			"start_date": date_iso or "",
			"end_date": "",
			"source": "columbia",
			"source_group": "columbia_history",
			"metadata": {
				"source_url": BASE_URL,
				"source_name": "Columbia University Department of History",
				"venue": {
...
def main():
	try:
		result = scrape_columbia_history_events()
		print(f"Columbia History events scraped: {len(result.get('events', []))}")
		# Save debug file
		if result.get("events"):
			import json
			with open("events_test/columbia_history_events.json", "w", encoding="utf-8") as f:
				json.dump(result, f, ensure_ascii=False, indent=2)
	except Exception as e:
		print(f"Error scraping Columbia History: {e}")
