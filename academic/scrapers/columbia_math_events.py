...
DATE_REGEXES = [
	...
	# e.g., 2016 SPRING TERM LECTURE SERIES
	re.compile(r"(\d{4})\s+([A-Z]+)\s+TERM"),
]

BASE_URL = "https://math.columbia.edu/events"
...
def scrape_columbia_math_events() -> Dict[str, List[Dict]]:
...
def main():
	try:
		result = scrape_columbia_math_events()
		print(f"Columbia Math events scraped: {len(result.get('events', []))}")
		# Save debug file
		if result.get("events"):
			import json
			with open("events_test/columbia_math_events.json", "w", encoding="utf-8") as f:
				json.dump(result, f, ensure_ascii=False, indent=2)
	except Exception as e:
		print(f"Error scraping Columbia Math: {e}")
