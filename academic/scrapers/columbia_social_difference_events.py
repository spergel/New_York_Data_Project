...
BASE_URL = "https://socialdifference.columbia.edu/events"
...
def scrape_columbia_social_difference_events() -> Dict[str, List[Dict]]:
	...
	for h in headers:
		...
		events.append({
			"id": f"evt_columbia_social_diff_{uid}",
			"name": title,
			"description": description,
			"start_date": date_iso or "",
			"end_date": "",
			"source": "columbia",
			"source_group": "columbia_social_difference",
			"metadata": {
				"source_url": BASE_URL,
				"source_name": "Columbia Center for Social Difference",
				"venue": {
...
def main():
	try:
		result = scrape_columbia_social_difference_events()
		print(f"Columbia Social Difference events scraped: {len(result.get('events', []))}")
		# Save debug file
