...
BASE_URL = "https://hunter.cuny.edu/events/"
...
def scrape_hunter_college_events() -> Dict[str, List[Dict]]:
...
def main():
	try:
		result = scrape_hunter_college_events()
		print(f"Hunter College events scraped: {len(result.get('events', []))}")
		# Save debug file
		if result.get("events"):
			import json
			with open("events_test/hunter_college_events.json", "w", encoding="utf-8") as f:
