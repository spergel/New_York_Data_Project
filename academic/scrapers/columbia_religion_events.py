...
BASE_URL = "https://ircpl.columbia.edu/events"
...
def scrape_columbia_religion_events() -> Dict[str, List[Dict]]:
	...
	for h in headers:
		...
		events.append({
			"id": f"evt_columbia_religion_{uid}",
			"name": title,
			"description": description,
			"start_date": date_iso or "",
			"end_date": "",
			"source": "columbia",
			"source_group": "columbia_religion",
			"metadata": {
				"source_url": BASE_URL,
				"source_name": "Columbia Institute for Religion, Culture, and Public Life",
				"venue": {
...
def main():
	try:
		result = scrape_columbia_religion_events()
		print(f"Columbia Religion events scraped: {len(result.get('events', []))}")
		# Save debug file
