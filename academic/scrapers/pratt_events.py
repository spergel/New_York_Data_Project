...
def scrape_pratt_events() -> Dict[str, List[Dict]]:
...
def main():
	try:
		result = scrape_pratt_events()
		print(f"Pratt events scraped: {len(result.get('events', []))}")
		# Save debug file
		if result.get("events"):
			import json
			with open("events_test/pratt_events.json", "w", encoding="utf-8") as f:
				json.dump(result, f, ensure_ascii=False, indent=2)
	except Exception as e:
		print(f"Error scraping Pratt: {e}")
