...
BASE_URL = "https://www.stjohns.edu/events"
...
def scrape_stjohns_events() -> Dict[str, List[Dict]]:
	...
	for h in headers:
		...
		events.append({
			"id": f"evt_stjohns_{uid}",
			"name": title,
			"description": description,
			"start_date": date_iso or "",
			"end_date": "",
			"source": "stjohns",
			"source_group": "stjohns",
			"metadata": {
				"source_url": BASE_URL,
				"source_name": "St. John's University",
				"venue": {
...
def main():
	try:
		result = scrape_stjohns_events()
		print(f"St. John's events scraped: {len(result.get('events', []))}")
		# Save debug file
		if result.get("events"):
			import json
			with open("events_test/stjohns_events.json", "w", encoding="utf-8") as f:
				json.dump(result, f, ensure_ascii=False, indent=2)
	except Exception as e:
		print(f"Error scraping St. John's: {e}")
