...
BASE_URL = "https://med.nyu.edu/events"
...
def scrape_nyu_medicine_events() -> Dict[str, List[Dict]]:
...
def main():
	try:
		result = scrape_nyu_medicine_events()
		print(f"NYU Medicine events scraped: {len(result.get('events', []))}")
		# Save debug file
		if result.get("events"):
			import json
			with open("events_test/nyu_medicine_events.json", "w", encoding="utf-8") as f:
				json.dump(result, f, ensure_ascii=False, indent=2)
	except Exception as e:
		print(f"Error scraping NYU Medicine: {e}")
