...
BASE_URL = "https://www.brooklyn.cuny.edu/events"
...
def scrape_brooklyn_college_events() -> Dict[str, List[Dict]]:
	...
	for h in headers:
		...
		events.append({
			"id": f"evt_brooklyn_{uid}",
			"name": title,
			"description": description,
			"start_date": date_iso or "",
			"end_date": "",
			"source": "brooklyn",
			"source_group": "brooklyn_college",
			"metadata": {
				"source_url": BASE_URL,
				"source_name": "Brooklyn College",
				"venue": {
...
def main():
	try:
		result = scrape_brooklyn_college_events()
		print(f"Brooklyn College events scraped: {len(result.get('events', []))}")
		# Save debug file
		if result.get("events"):
			import json
			with open("events_test/brooklyn_college_events.json", "w", encoding="utf-8") as f:
				json.dump(result, f, ensure_ascii=False, indent=2)
	except Exception as e:
		print(f"Error scraping Brooklyn College: {e}")
